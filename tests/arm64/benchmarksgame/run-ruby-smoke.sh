...
REPORT="$REPORT_DIR/benchmarksgame-ruby-smoke-$STAMP.md"
GUEST_WORK="/tmp/benchmarksgame-ruby-smoke"
HOST_TMP="$(mktemp -d)"
...
fetch_sources() {
    log "Fetching Benchmarks Game Ruby sources"
...
for bench in benches:
    ...
    pages = re.findall(r'\.\./program/(' + re.escape(bench) + r'-ruby-[^\"]+)', perf)
    if not pages:
        raise RuntimeError(f"no Ruby page for {bench}")
    chosen_page = None
    chosen_source = None
    skipped = []
    for page in pages:
    ...
    (src_dir / f"{bench}.rb").write_text(chosen_source)
...
cd /tmp/benchmarksgame-ruby-smoke
mkdir -p out
...
run_arg() {
    ...
    /usr/bin/time -f "__BG_TIME:$name:%e" ruby "src/$name.rb" "$arg" > "out/$name.out"
...
run_stdin() {
    name="$1"
    echo "__BG_BEGIN:$name"
    /usr/bin/time -f "__BG_TIME:$name:%e" ruby "src/$name.rb" < input.fa > "out/$name.out"
    bytes=$(wc -c < "out/$name.out"); lines=$(wc -l < "out/$name.out")
    cksum=$(cksum "out/$name.out" | awk '{print $1":"$2}')
    echo "__BG_RESULT:$name:PASS:$bytes:$lines:$cksum"
}
...
run_guest() {
    log "Running Ruby Benchmarks Game smoke in guest"
...
write_report() {
    ...
    {
        echo "# Benchmarks Game Ruby smoke report"
        echo
        ...
        echo
        echo "## Selected Ruby source variants"
        echo
