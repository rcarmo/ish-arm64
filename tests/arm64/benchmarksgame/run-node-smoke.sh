...
REPORT="$REPORT_DIR/benchmarksgame-node-smoke-$STAMP.md"
GUEST_WORK="/tmp/benchmarksgame-node-smoke"
HOST_TMP="$(mktemp -d)"
...
fetch_sources() {
    log "Fetching Benchmarks Game Node.js sources"
...
# Keep worker_threads out of the first Node row so this lane stays single-process.
...
for bench in benches:
    ...
    pages = re.findall(r'\.\./program/(' + re.escape(bench) + r'-node-[^\"]+)', perf)
    if not pages:
        raise RuntimeError(f"no Node page for {bench}")
    chosen_page = None
    chosen_source = None
    skipped = []
    for page in pages:
    ...
    (src_dir / f"{bench}.js").write_text(chosen_source)
...
cd /tmp/benchmarksgame-node-smoke
mkdir -p out
...
run_arg() {
    ...
    /usr/bin/time -f "__BG_TIME:$name:%e" node "src/$name.js" "$arg" > "out/$name.out"
...
run_stdin() {
    name="$1"
    echo "__BG_BEGIN:$name"
    /usr/bin/time -f "__BG_TIME:$name:%e" node "src/$name.js" < input.fa > "out/$name.out"
    bytes=$(wc -c < "out/$name.out"); lines=$(wc -l < "out/$name.out")
    cksum=$(cksum "out/$name.out" | awk '{print $1":"$2}')
    echo "__BG_RESULT:$name:PASS:$bytes:$lines:$cksum"
}
...
run_guest() {
    log "Running Node.js Benchmarks Game smoke in guest"
...
write_report() {
    ...
    {
        echo "# Benchmarks Game Node.js smoke report"
        echo
        ...
        echo
        echo "## Selected Node.js source variants"
        echo
