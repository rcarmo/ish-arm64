...
REPORT="$REPORT_DIR/benchmarksgame-perl-smoke-$STAMP.md"
GUEST_WORK="/tmp/benchmarksgame-perl-smoke"
HOST_TMP="$(mktemp -d)"
...
fetch_sources() {
    log "Fetching Benchmarks Game Perl sources"
...
external_markers = (
    "require('mpzjs')", 'require("mpzjs")',
    "require('ffi", 'require("ffi',
)
# Keep worker_threads out of the first Perl row so this lane stays single-process.
# Add a separate worker-thread row later because it is a useful scheduler/futex stressor.
skip_markers = external_markers + ("worker_threads",)

for bench in benches:
    perf = fetch(f"{site}/performance/{bench}.html")
    pages = re.findall(r'\.\./program/(' + re.escape(bench) + r'-perl-[^\"]+)', perf)
    if not pages:
        raise RuntimeError(f"no Perl page for {bench}")
    chosen_page = None
    chosen_source = None
    skipped = []
    for page in pages:
        source = source_from_page(page)
        hits = [m for m in skip_markers if m in source]
        if hits:
            skipped.append(page + ":" + "/".join(hits))
            continue
        # Alpine does not package Math::BigInt::GMP. Keep pidigits in the
        # first Perl row by using the stdlib Math::BigInt backend and recording
        # this as an adaptation in the report.
        if bench == "pidigits" and "Math::BigInt lib => 'GMP'" in source:
            source = source.replace("use Math::BigInt lib => 'GMP';", "use Math::BigInt;")
            source = source.replace("\n\ndie 'Math::BigInt::GMP missing!'\n    if Math::BigInt->config->{lib} ne 'Math::BigInt::GMP';", "")
            skipped.append(page + ":adapted-no-Math::BigInt::GMP")
        chosen_page = page
    ...
    (src_dir / f"{bench}.pl").write_text(chosen_source)
...
cd /tmp/benchmarksgame-perl-smoke
mkdir -p out
...
run_arg() {
    ...
    /usr/bin/time -f "__BG_TIME:$name:%e" perl "src/$name.pl" "$arg" > "out/$name.out"
...
run_stdin() {
    name="$1"
    echo "__BG_BEGIN:$name"
    /usr/bin/time -f "__BG_TIME:$name:%e" perl "src/$name.pl" < input.fa > "out/$name.out"
    bytes=$(wc -c < "out/$name.out"); lines=$(wc -l < "out/$name.out")
    cksum=$(cksum "out/$name.out" | awk '{print $1":"$2}')
    echo "__BG_RESULT:$name:PASS:$bytes:$lines:$cksum"
}
...
run_guest() {
    log "Running Perl Benchmarks Game smoke in guest"
...
write_report() {
    ...
    {
        echo "# Benchmarks Game Perl smoke report"
        echo
        ...
        echo
        echo "## Selected Perl source variants"
        echo
        echo "| Benchmark | Program page | Skipped/adapted alternatives |"
        echo "|---|---|---|"
