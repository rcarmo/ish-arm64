...
REPORT="$REPORT_DIR/benchmarksgame-php-smoke-$STAMP.md"
GUEST_WORK="/tmp/benchmarksgame-php-smoke"
HOST_TMP="$(mktemp -d)"
...
ensure_guest_packages() {
    log "Ensuring PHP runtime/extensions in guest"
    guest_capture "apk add --no-cache php84 php84-gmp php84-pcntl php84-mbstring php84-shmop php84-sysvmsg php84-sysvsem php84-sysvshm >/dev/null && php -m | grep -Eq '^gmp$' && php -m | grep -Eq '^pcntl$' && php -m | grep -Eq '^shmop$'" >/dev/null
}

fetch_sources() {
    log "Fetching Benchmarks Game PHP sources"
...
# Prefer the official order from the performance page. The guest installs the
# Alpine-packaged extensions needed by the fastest PHP programs (GMP/pcntl), so
# these are not treated as external skips.
external_markers = ("FFI::", "pthreads")

for bench in benches:
    perf = fetch(f"{site}/performance/{bench}.html")
    pages = re.findall(r'\.\./program/(' + re.escape(bench) + r'-php-[^\"]+)', perf)
    if not pages:
        raise RuntimeError(f"no PHP page for {bench}")
    chosen_page = None
    chosen_source = None
    skipped = []
    for page in pages:
    ...
    (src_dir / f"{bench}.php").write_text(chosen_source)
...
cd /tmp/benchmarksgame-php-smoke
mkdir -p out
...
run_arg() {
    ...
    /usr/bin/time -f "__BG_TIME:$name:%e" php -d memory_limit=-1 "src/$name.php" "$arg" > "out/$name.out"
...
run_stdin() {
    name="$1"
    echo "__BG_BEGIN:$name"
    /usr/bin/time -f "__BG_TIME:$name:%e" php -d memory_limit=-1 "src/$name.php" < input.fa > "out/$name.out"
    bytes=$(wc -c < "out/$name.out"); lines=$(wc -l < "out/$name.out")
    cksum=$(cksum "out/$name.out" | awk '{print $1":"$2}')
    echo "__BG_RESULT:$name:PASS:$bytes:$lines:$cksum"
}
...
run_guest() {
    log "Running PHP Benchmarks Game smoke in guest"
...
write_report() {
    ...
    {
        echo "# Benchmarks Game PHP smoke report"
        echo
        ...
        echo
        echo "## Selected PHP source variants"
        echo
