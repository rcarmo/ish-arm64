...
REPORT="$REPORT_DIR/benchmarksgame-lua-smoke-$STAMP.md"
GUEST_WORK="/tmp/benchmarksgame-lua-smoke"
HOST_TMP="$(mktemp -d)"
...
ensure_guest_packages() {
    log "Ensuring Lua runtime/modules in guest"
    guest_capture "apk add --no-cache lua5.3 lua5.3-dev luarocks5.3 lua5.3-rex-pcre2 gmp-dev gcc musl-dev make >/dev/null && (lua5.3 -e 'local g={}; require(\"c-gmp\")(g,{}); assert(g.mpz_init_set_d)' >/dev/null 2>&1 || luarocks-5.3 install lgmp >/dev/null) && lua5.3 -e 'require(\"rex_pcre2\")'" >/dev/null
}

fetch_sources() {
    log "Fetching Benchmarks Game Lua sources"
...
# The pidigits official Lua variants require either LGMP (c-gmp) or the bn
# module. Alpine can build LGMP with Lua 5.3, but LGMP explicitly does not
# support Lua 5.4, so this row runs official Lua sources under lua5.3.
external_markers = ('require"bn"', "require\"bn\"", "require 'bn'", "require('bn')")

for bench in benches:
    perf = fetch(f"{site}/performance/{bench}.html")
    pages = re.findall(r'\.\./program/(' + re.escape(bench) + r'-lua-[^\"]+)', perf)
    if not pages:
        raise RuntimeError(f"no Lua page for {bench}")
    chosen_page = None
    chosen_source = None
    skipped = []
    for page in pages:
        source = source_from_page(page)
        hits = [m for m in external_markers if m in source]
        if hits:
            skipped.append(page + ":" + "/".join(hits))
            continue
        chosen_page = page
    ...
    (src_dir / f"{bench}.lua").write_text(chosen_source)
...
cd /tmp/benchmarksgame-lua-smoke
mkdir -p out
...
run_arg() {
    ...
    /usr/bin/time -f "__BG_TIME:$name:%e" lua5.3 "src/$name.lua" "$arg" > "out/$name.out"
...
run_stdin() {
    name="$1"
    echo "__BG_BEGIN:$name"
    /usr/bin/time -f "__BG_TIME:$name:%e" lua5.3 "src/$name.lua" < input.fa > "out/$name.out"
    bytes=$(wc -c < "out/$name.out"); lines=$(wc -l < "out/$name.out")
    cksum=$(cksum "out/$name.out" | awk '{print $1":"$2}')
    echo "__BG_RESULT:$name:PASS:$bytes:$lines:$cksum"
}
...
run_guest() {
    log "Running Lua Benchmarks Game smoke in guest"
...
write_report() {
    ...
    {
        echo "# Benchmarks Game Lua smoke report"
        echo
        ...
        echo
        echo "## Selected Lua source variants"
        echo
        echo "| Benchmark | Program page | Skipped external-dependency alternatives |"
        echo "|---|---|---|"
        ...
        echo
        echo '```text'
        tail -160 "$HOST_TMP/guest.log" | sed '/^__ISH_STATUS:/d'
        echo '```'
    } >"$REPORT"
    echo "report: $REPORT"
