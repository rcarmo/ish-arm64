...
REPORT="$REPORT_DIR/benchmarksgame-python-smoke-$STAMP.md"
GUEST_WORK="/tmp/benchmarksgame-python-smoke"
HOST_TMP="$(mktemp -d)"
...
fetch_sources() {
    log "Fetching Benchmarks Game Python sources"
...
external_markers = (
    ...
    "import gmpy", "from gmpy",
    "import regex", "from regex",
)

for bench in benches:
    perf = fetch(f"{site}/performance/{bench}.html")
    pages = re.findall(r'\.\./program/(' + re.escape(bench) + r'-python3-[^\"]+)', perf)
    if not pages:
        raise RuntimeError(f"no Python page for {bench}")
    chosen_page = None
    chosen_source = None
    skipped = []
    for page in pages:
    ...
    (src_dir / f"{bench}.py").write_text(chosen_source)
...
run_arg() {
    ...
    /usr/bin/time -f "__BG_TIME:$name:%e" python3 "src/$name.py" "$arg" > "out/$name.out"
...
run_stdin() {
    name="$1"
    echo "__BG_BEGIN:$name"
    /usr/bin/time -f "__BG_TIME:$name:%e" python3 "src/$name.py" < input.fa > "out/$name.out"
    bytes=$(wc -c < "out/$name.out"); lines=$(wc -l < "out/$name.out")
    cksum=$(cksum "out/$name.out" | awk '{print $1":"$2}')
    echo "__BG_RESULT:$name:PASS:$bytes:$lines:$cksum"
}
...
run_guest() {
    log "Running Python Benchmarks Game smoke in guest"
